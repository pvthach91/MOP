 ...
 **/
var IMAGESPATH = '/images/popup/';
var _SCROLLINGJXF = "auto";
var TopImageSign = '/images/popup/icon_dialog.png';
...
Dialog.prototype.create = function () {
  ...
  <table id="_DialogTable_{thisID}" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" style="font-size:12px; line-height:1.4;border-collapse: collapse;">\
    <tr id="_Draghandle_{thisID}" onselectstart="return false;" style="-moz-user-select: -moz-none; ' + (this.Drag ? "cursor: move;" : "") + '">\
      ...
      <td height="33" style="background-image:url({IMAGESPATH}dialog_ct.png) !important;background: url({IMAGESPATH}dialog_ct.gif) repeat-x top;"><div style="padding: 7px 5px 0 0; float: left; font-weight: bold; color:#fff;"><img align="absmiddle" src="{TopImageSign}"/> &nbsp;<span id="_Title_{thisID}">' + this.Title + '</span></div>\
	...
	' + (function (obj) {
        ...
        if (obj.URL) return '<iframe width="100%" scrolling="'+_SCROLLINGJXF+'" height="100%" frameborder="0" style="border:none 0;" id="_DialogFrame_' + obj.ID + '" ' + (obj.InnerFrameName?'name="'+obj.InnerFrameName+'"':'')+' src="' + obj.displacePath() + '"></iframe>';
        return "";
    })(this) + '\
              </div></td>\
          </tr>\
...
';
	html=html.replace(/\{IMAGESPATH\}/gm,IMAGESPATH).replace(/\{thisID\}/gm,this.ID).replace(/\{TopImageSign\}/gm,TopImageSign);
